describe('Login Page', () => {
    ...
    beforeEach(() => {
      cy.visit('https://admin:nimda@betizy.io/?modal=login'); // Open the login page
    });
    ...
    // Click the submit button using the specific class
        cy.get('button._secondary_i1e1t_9[type="Login now"]').click();
  
  
    // Verify that the user is redirected to the correct page
        cy.url().should('include', 'https://betizy.io/'); // Adjust the expected URL as needed
    });
    ...
    it('6. should redirect to the registration page when "Register Now" button is clicked', () => {
        ...
        cy.url().should('include', 'https://betizy.io/?modal=login'); // 
    });
